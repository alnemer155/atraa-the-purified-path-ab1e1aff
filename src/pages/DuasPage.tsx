      ...
      <div className="animate-fade-in min-h-[calc(100vh-130px)] flex flex-col">
        <div className="sticky top-[42px] z-30 bg-background/70 backdrop-blur-2xl px-4 py-3 border-b border-border/15">
          <div className="flex items-center justify-between">
            <button onClick={() => setSelectedItem(null)} className="flex items-center gap-1.5 text-primary text-sm group">
              <ChevronRight className="w-4 h-4" />
              رجوع
            </button>
            <div className="flex items-center gap-1">
              <button onClick={() => setFontSize(s => Math.max(14, s - 2))}
                className="w-8 h-8 rounded-xl bg-secondary/50 text-foreground text-xs flex items-center justify-center active:scale-95">
                أ−
              </button>
              <button onClick={() => setFontSize(s => Math.min(32, s + 2))}
                className="w-8 h-8 rounded-xl bg-secondary/50 text-foreground text-sm flex items-center justify-center active:scale-95">
                أ+
              </button>
            </div>
          </div>
        ...
        <div className="flex-1 px-5 py-5">
          <div className="mb-5">
            <span className="inline-flex items-center gap-1 text-[10px] text-primary tracking-wider bg-primary/8 px-2.5 py-1 rounded-lg mb-2">
              {categoryLabels[selectedItem.category]}
            </span>
            <h1 className="text-xl text-foreground leading-snug tracking-tight">{selectedItem.title}</h1>
            <p className="text-[10px] text-muted-foreground/60 mt-1.5 font-light">المصدر: حقيبة المؤمن</p>
          </div>
          <div
        ...
        <div className="sticky bottom-[76px] px-5 py-3 flex items-center justify-between bg-background/70 backdrop-blur-2xl border-t border-border/15">
          <button onClick={() => hasNext && handleSelectItem(filtered[currentIndex + 1])} disabled={!hasNext}
            className="flex items-center gap-1.5 text-sm text-primary disabled:opacity-20 active:scale-95 transition-transform">
            التالي <ChevronLeft className="w-3.5 h-3.5" />
          </button>
          <span className="text-[10px] text-muted-foreground/50 tabular-nums bg-secondary/30 px-3 py-1 rounded-full">
  ...
  return (
    <div className="px-4 py-5 animate-fade-in">
        ...
        {categories.map(cat => {
          ...
          return (
            <button
              ...
              className={`flex-1 py-3 px-2 rounded-2xl text-center transition-all relative active:scale-[0.97] ${
                isActive
                  ? 'bg-foreground text-background'
                  : 'bg-card border border-border/20 text-foreground'
              }`}
            >
              <span className="text-[12px] block">{cat.label}</span>
              <span className={`text-[9px] font-light block mt-0.5 ${isActive ? 'text-background/60' : 'text-muted-foreground/60'}`}>
                {count} عنصر
              </span>
            </button>
          );
      ...
      <div className="relative mb-4">
        <div className="flex items-center gap-2.5 bg-card/80 backdrop-blur-sm border border-border/25 rounded-2xl px-4 py-3 focus-within:border-primary/30 transition-all">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-sm text-foreground placeholder:text-muted-foreground/40 outline-none font-light"
          />
          {search && (
        ...
        >
          {filtered.map((item, i) => (
            <motion.button
              ...
              className="w-full text-right p-4 rounded-2xl bg-card/80 backdrop-blur-sm border border-border/20 hover:border-primary/25 transition-all group active:scale-[0.98]"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] text-foreground group-hover:text-primary transition-colors leading-snug">{item.title}</p>
                  <p className="text-[11px] text-muted-foreground/60 mt-1.5 line-clamp-2 leading-relaxed font-light">
                    {item.content.slice(0, 100)}...
                  </p>
                </div>
                <ChevronLeft className="w-3.5 h-3.5 text-primary/40 group-hover:text-primary transition-colors flex-shrink-0 mt-0.5" />
              </div>
            </motion.button>
          ))}
          {filtered.length === 0 && (
            <div className="text-center py-16">
              <Search className="w-7 h-7 text-muted-foreground/30 mx-auto mb-4" />
              <p className="text-sm text-muted-foreground/50">لا توجد نتائج</p>
              <p className="text-[11px] text-muted-foreground/30 mt-1 font-light">جرّب كلمة بحث مختلفة</p>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
